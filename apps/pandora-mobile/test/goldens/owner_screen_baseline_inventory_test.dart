...
const _manifestPath =
    'docs/testing/owner-screen-baseline-candidate-manifest.txt';
...
void main() {
  test('production baseline inventory is exact and provenance-bound', () {
    final directory = Directory(_baselineDirectory);
    expect(
      directory.existsSync(),
      isTrue,
      reason: 'Production-screen baselines must be committed before CI runs.',
    );
    ...
    for (final name in _expectedBaselines) {
      ...
      expect(
        bytes.length,
        greaterThan(1024),
        reason: '$name must contain a non-empty production-screen capture.',
      );
    ...
    final manifestedBaselines = lines
        .map(hashLine.firstMatch)
        .whereType<RegExpMatch>()
        .map((match) => match.group(2)!)
        .toSet();
    expect(manifestedBaselines, equals(_expectedBaselines));
  });
}
