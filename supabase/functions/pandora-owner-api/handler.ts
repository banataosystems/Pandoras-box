...
async function authenticate(req: Request, env: EnvDeps): Promise<UserContext> {
  const authorization = req.headers.get("authorization") || "";
  if (!/^Bearer\s+\S+$/i.test(authorization)) {
    throw new Error("SIGN_IN_REQUIRED");
  }

  const client = env.createClient(env.SUPABASE_URL, env.SUPABASE_ANON_KEY, {
...
async function enforceRateLimit(context: UserContext, method: string, env: EnvDeps) {
  const admin = env.createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY, {
    auth: { persistSession: false, autoRefreshToken: false },
  });
...
import { validateBrokerOrigin } from "../../src/projectos/broker-origin-validator.ts";
...
async function acceptIntake(
  ...
  const projectosClient = {
    ...
    createExecutionPlan: async (data: any) => {
      const { data: resultData, error } = await adminClient.rpc("create_execution_plan", {
        p_organization_id: data.organizationId,
        p_request_id: data.requestId,
        p_intake_id: data.intakeId,
        p_tool: data.tool,
        p_risk: data.risk,
        p_args: data.args,
        p_payload_hash: data.payloadHash,
        p_expires_at: data.expiresAt
      });
      if (error) throw new Error(`CREATE_PLAN_FAILED: ${error.message}`);
      return { planId: resultData.planId, status: resultData.status };
    }
  };
  ...
  const providerRunner = {
    execute: async ({ intakeId }: { intakeId: string }) => {
      ...
      const planId = analysis?.latestExecutionPlanId;
      if (!planId) {
      ...
      const { data: planData, error: planError } = await adminClient.rpc("resolve_execution_plan_securely", {
        p_plan_id: planId
      });
      if (planError || !planData) throw new Error("PLAN_NOT_FOUND");
      if (planData.organization_id !== context.organizationId) {
      ...
      try {
        ...
        const brokerUrl = validation.url;
      ...
      const payloadHash = claimData?.payloadHash || claimData?.payload_hash;
      if (!payloadHash) {
         throw new Error("PROVIDER_VERIFICATION_FAILED: No payload hash bound to execution claim");
      }
...
export async function handleOwnerApiRequest(req: Request, env: EnvDeps): Promise<Response> {
  const requestId = crypto.randomUUID();
  const requestOrigin = req.headers.get("origin");
  const corsOrigin = allowedCorsOrigin(requestOrigin, env.ALLOWED_ORIGINS);
  ...
  try {
    const context = await authenticate(req, env);
    const url = new URL(req.url);
    const route = normalizeOwnerRoute(url.pathname);
    await enforceRateLimit(context, req.method, env);
