...
create or replace function public.get_github_control_accounts(p_organization_id uuid)
returns jsonb
language plpgsql
security definer
set search_path = ''
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'baseUrl', coalesce(nullif(installation.configuration ->> 'api_origin', ''), 'https://api.github.com'),
        'login', nullif(installation.configuration ->> 'login', ''),
        'allowedRepositories', coalesce(installation.configuration -> 'allowed_repositories', '[]'::jsonb),
        'grantedScopes', to_jsonb(coalesce(installation.scopes, '{}'::text[]))
      )
...
revoke all on function public.get_github_control_accounts(uuid) from public, anon, authenticated;
grant execute on function public.get_github_control_accounts(uuid) to service_role;
