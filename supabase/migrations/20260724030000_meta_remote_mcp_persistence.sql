...
-- Reconstructed schema-shape hypothesis; original bytes are unavailable.
-- Current live catalog shape is separately content-addressed in the recovery manifest.
-- Positive grants are intentionally deferred to their later historical migrations.
...
create table public.meta_drafts (
  ...
  kind text not null check (
    kind in ('post', 'comment_reply', 'message_reply', 'weekly_plan')
  ),
  target_id text check (
    target_id is null or char_length(target_id) between 1 and 256
  ),
...
create trigger meta_webhook_health_set_updated_at
  before update on public.meta_webhook_health
  for each row execute function private.set_updated_at();
