...
for (const [name, resultFactory] of [
  ...
  test(`provider success plus ${name} is completed for reconciliation, never durably failed`, async () => {
...
test("an unclassified post-dispatch error fails closed as ambiguous for a non-idempotent mutation", async () => {
  ...
  assert.equal(ledger.finishInputs[0].status, "completed");
  assert.equal(ledger.finishInputs[0].resultSummary.providerOutcome, "ambiguous");
});

test("408 and 429 remain retryable only when provider execution is known not to have occurred", async () => {
...
test("a provider failure explicitly proven before side effects remains a truthful failed plan", async () => {
  ...
  const handler = createProjectOsMcpHandler(dependencies({
    ledger,
    async execute() {
      throw Object.assign(new Error("validation rejected before dispatch"), {
        status: 422,
...
test("ambiguous provider-supported replay reuses one immutable idempotency identity", async () => {
  ...
  assert.equal(
    firstLedger.finishInputs[0].resultSummary.idempotencyIdentityHash,
...
test("sensitive provider payload is never persisted in reconciliation evidence", async () => {
  ...
  assert.equal(ledger.finishInputs[0].resultSummary.evidencePolicy, "privacy_safe_summary_only_v1");
});
