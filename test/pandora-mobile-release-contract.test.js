...
test('Pandora mobile package identity and AndroidManifest are consistent', () => {
  ...
  assert.match(pubspec, /version:\s*0\.3\.0-rc\.2\+2/);
  ...
  assert.match(config, /appVersion\s*=\s*'0\.3\.0-rc\.2'/);
  assert.match(config, /buildNumber\s*=\s*'2'/);
