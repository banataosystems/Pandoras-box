...
const migration = fs.readFileSync(
  new URL('../supabase/migrations/20260820000500_pandora_member_access_control.sql', import.meta.url),
  'utf8',
);
const approvalCompat = fs.readFileSync(
  new URL('../supabase/migrations/20260820000600_pandora_approval_permission_compat.sql', import.meta.url),
  'utf8',
);
...
const accessApi = fs.readFileSync(
  new URL('../supabase/functions/pandora-user-access-api/index.ts', import.meta.url),
  'utf8',
);
const mobileConfig = fs.readFileSync(
  new URL('../apps/pandora-mobile/lib/pandora_config.dart', import.meta.url),
  'utf8',
);
const settings = fs.readFileSync(
  new URL('../apps/pandora-mobile/lib/features/settings/settings_screen.dart', import.meta.url),
  'utf8',
);
...
test('fine-grained access schema is fail-closed and preserves history', () => {
  for (const table of [
    ...
    'membership_project_access',
    'membership_access_audit',
  ]) {
  ...
  assert.match(migration, /membership_project_access[\s\S]*project_id uuid/i);
  ...
  assert.match(migration, /membership_access_audit/i);
  assert.doesNotMatch(
    migration,
    /grant\s+(?:insert|update|delete|all)[^;]*membership_permissions[^;]*authenticated/i,
  );
});
...
test('owner API no longer uses a coarse owner-admin wall', () => {
  assert.doesNotMatch(ownerApi, /OWNER_ROLE_REQUIRED/);
