export type OwnerConnectionState = 'ready' | 'problem' | 'off' | 'needs_permission' | 'not_checked';
...
export class PandoraOwnerApiClient {
  ...
  constructor(options: {
    ...
    if (!/^https:\/\//i.test(normalized)) {
      throw new PandoraOwnerApiError('Pandora connection must use HTTPS.', 0, 'HTTPS_REQUIRED');
    }
  ...
  project<T = unknown>(projectId: string): Promise<T> {
    return this.request<T>(`/api/owner/projects/${encodeURIComponent(requireValue(projectId, 'projectId'))}`);
  }
  ...
  runAction<T = unknown>(actionId: string): Promise<T> {
    return this.request<T>(`/api/owner/actions/${encodeURIComponent(requireValue(actionId, 'actionId'))}/run`, {
      method: 'POST',
      body: JSON.stringify({ clientMode: 'simple' }),
    });
  }

  decideApproval<T = unknown>(approvalId: string, decision: 'approve' | 'reject'): Promise<T> {
    return this.request<T>(`/api/owner/approvals/${encodeURIComponent(requireValue(approvalId, 'approvalId'))}/decide`, {
      method: 'POST',
  ...
  private async request<T>(path: string, init: RequestInit = {}): Promise<T> {
    ...
    if (!token) {
      throw new PandoraOwnerApiError('Please sign in again.', 401, 'SIGN_IN_REQUIRED');
    }
    ...
    if (!response.ok) {
      const plainMessage =
        typeof payload?.plainMessage === 'string'
      ...
      const code = typeof payload?.code === 'string' ? payload.code : 'OWNER_API_REQUEST_FAILED';
...
function requireValue(value: string, name: string): string {
  ...
  if (!trimmed) throw new PandoraOwnerApiError(`${name} is required.`, 0, 'INVALID_INPUT');
  return trimmed;
}
