...
import { PandoraOwnerApiClient, PandoraOwnerApiError } from './pandora-owner-api-client.ts';
...
test('approval decision sends only the governed decision', async () => {
  ...
  assert.equal(seenUrl, 'https://pandora.example/api/owner/approvals/approval%20123/decide');
  assert.deepEqual(body, { decision: 'approve', clientMode: 'simple' });
});

test('non-HTTPS base URL is rejected', () => {
  assert.throws(
    () => new PandoraOwnerApiClient({ baseUrl: 'http://pandora.example', getAccessToken: () => 'x' }),
    (error: unknown) => error instanceof PandoraOwnerApiError && error.code === 'HTTPS_REQUIRED',
  );
});
